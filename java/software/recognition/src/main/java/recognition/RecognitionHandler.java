        ...
        try {
            for (var eventRecord : event.getRecords()) {
                ...
                for (final var record : root.get("Records") ) {
                    ...
                    final var labels = ImageRecognizer.detectLabels(bucketName, key);
                    
                    // <<Amazon CodeWhisperer generated code goes here>>

                    System.out.println("Image: " + key);
                    ...
                    LabelStorage.storeLabels(dbItem);
                    // <<Amazon CodeWhisperer generated code goes here>>
            
                    // call the LabelStorage method method #2 to send message to SNS topic
                    LabelStorage.sendNotification(labels, topicArn);          
                    // <<Amazon CodeWhisperer generated code goes here>>

                    // call the LabelStorage method method #3 to delete img from SQS queue
                    LabelStorage.deleteMessage(receiptHandle, queueUrl);
